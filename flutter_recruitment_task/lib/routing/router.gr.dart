// **************************************************************************
// AutoRouteGenerator
// **************************************************************************

// GENERATED CODE - DO NOT MODIFY BY HAND

// **************************************************************************
// AutoRouteGenerator
// **************************************************************************
//
// ignore_for_file: type=lint

import 'package:auto_route/auto_route.dart' as _i4;
import 'package:flutter/material.dart' as _i5;

import '../models/movie.dart' as _i6;
import '../pages/movie_details/movie_details_page.dart' as _i2;
import '../pages/movie_list/movie_list_page.dart' as _i1;
import '../pages/two_buttons/two_buttons_page.dart' as _i3;

class AppRouter extends _i4.RootStackRouter {
  AppRouter([_i5.GlobalKey<_i5.NavigatorState>? navigatorKey])
      : super(navigatorKey);

  @override
  final Map<String, _i4.PageFactory> pagesMap = {
    MovieListRoute.name: (routeData) {
      return _i4.MaterialPageX<dynamic>(
          routeData: routeData, child: _i1.MovieListPage());
    },
    MovieDetailsRoute.name: (routeData) {
      final args = routeData.argsAs<MovieDetailsRouteArgs>();
      return _i4.MaterialPageX<dynamic>(
          routeData: routeData,
          child: _i2.MovieDetailsPage(key: args.key, movie: args.movie));
    },
    TwoButtonsRoute.name: (routeData) {
      return _i4.MaterialPageX<dynamic>(
          routeData: routeData, child: _i3.TwoButtonsPage());
    }
  };

  @override
  List<_i4.RouteConfig> get routes => [
        _i4.RouteConfig(MovieListRoute.name, path: '/'),
        _i4.RouteConfig(MovieDetailsRoute.name, path: '/movie-details-page'),
        _i4.RouteConfig(TwoButtonsRoute.name, path: '/two-buttons-page')
      ];
}

/// generated route for
/// [_i1.MovieListPage]
class MovieListRoute extends _i4.PageRouteInfo<void> {
  const MovieListRoute() : super(MovieListRoute.name, path: '/');

  static const String name = 'MovieListRoute';
}

/// generated route for
/// [_i2.MovieDetailsPage]
class MovieDetailsRoute extends _i4.PageRouteInfo<MovieDetailsRouteArgs> {
  MovieDetailsRoute({_i5.Key? key, required _i6.Movie movie})
      : super(MovieDetailsRoute.name,
            path: '/movie-details-page',
            args: MovieDetailsRouteArgs(key: key, movie: movie));

  static const String name = 'MovieDetailsRoute';
}

class MovieDetailsRouteArgs {
  const MovieDetailsRouteArgs({this.key, required this.movie});

  final _i5.Key? key;

  final _i6.Movie movie;

  @override
  String toString() {
    return 'MovieDetailsRouteArgs{key: $key, movie: $movie}';
  }
}

/// generated route for
/// [_i3.TwoButtonsPage]
class TwoButtonsRoute extends _i4.PageRouteInfo<void> {
  const TwoButtonsRoute()
      : super(TwoButtonsRoute.name, path: '/two-buttons-page');

  static const String name = 'TwoButtonsRoute';
}
